module core #(parameter NUM_WARPS = 4)(
    input  wire clk,
    input  wire reset,
    ...
    genvar m;
    generate
        for(m=0; m<16; m=m+1) begin :warp0_assigns
            assign RS1_warps0[m] = RS1_warps[0][m];
            assign RS2_warps0[m] = RS2_warps[0][m];
        end
    endgenerate
    ...
    generate
        for(m=0; m<16; m=m+1) begin :warp1_assigns
            assign RS1_warps1[m] = RS1_warps[1][m];
            assign RS2_warps1[m] = RS2_warps[1][m];
        end
    endgenerate
    ...
    generate
        for(m=0; m<16; m=m+1) begin :warp2_assigns
            assign RS1_warps2[m] = RS1_warps[2][m];
            assign RS2_warps2[m] = RS2_warps[2][m];
        end
    endgenerate
    ...
    generate
        for(m=0; m<16; m=m+1) begin :warp3_assigns
            assign RS1_warps3[m] = RS1_warps[3][m];
            assign RS2_warps3[m] = RS2_warps[3][m];
        end
    endgenerate
    ...
    assign warp_done[0]   = halt && (current_warp_id == 0);
    assign warp_done[1]   = halt && (current_warp_id == 1);
